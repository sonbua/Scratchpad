...
let private httpVnEconomy = http { config_useBaseUrl baseUrl }

let private trimBaseUrl (url: string) =
    if url |> String.startsWith baseUrl then
        url |> String.drop (baseUrl |> String.length)
    else
        url

let extractBlogChungKhoanLinks url =
    async {
        let! response = httpVnEconomy { GET(url |> trimBaseUrl) } |> Request.sendAsync
